...
def version():
    return "21"
    # TODO add mqtt disconnect on graceful reboot
    #       Add local time keeping if no heartbeat?
    # 21: added wifi reconnect if lost (Spin()), close mqtt on reset and debug path fixed
...
class HassMqtt:
    ...
    def __init__(self, nodename, sensor):
        ...
        self.mqttreconnects = sensor.Device("mqttreconnects",'sensor', 0, units='connections')
        self.hour = sensor.Device("hour",'sensor',-1,attrs={"local":True})
        self.minute = sensor.Device("minute",'sensor',-1, attrs={"local":True})
        self.Connect()
        ...
        if self.connected:
            ...
            for sensor, object in self.sensor.list.items():
                if not 'local' in object.attrs:
                    ...
                    if clear:
                        self.client.publish("homeassistant" + node_topic + "/config", '', retain=True)
                    else:
